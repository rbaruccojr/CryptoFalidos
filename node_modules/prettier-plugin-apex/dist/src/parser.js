...
export default async function parse(sourceCode, options) {
    ...
    if (options.apexStandaloneParser === "built-in") {
        ...
        serializedAst = await parseTextWithSpawn(path.join(getSerializerBinDirectory(), `apex-ast-serializer${process.platform === "win32" ? ".bat" : ""}`), sourceCode, options.parser === "apex-anonymous");
    }
    if (serializedAst) {
